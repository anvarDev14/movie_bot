...
async def remove_admin_handler(message: types.Message):
    ...
    if not admins:
        ...
        await send_response(message, "❌ Hozircha hech qanday admin yo‘q.", admin_menu)
        return
    keyboard = InlineKeyboardMarkup(row_width=1)
    for admin in admins:
        admin_id = admin[0]
        admin_name = admin[1] if len(admin) > 1 else f"Admin {admin_id}"
        keyboard.add(InlineKeyboardButton(f"{admin_name} ({admin[2]})", callback_data=f"confirm_remove_admin:{admin_id}"))
...
async def confirm_remove_admin(callback_query: CallbackQuery):
    ...
    await send_response(callback_query, f"✅ Admin (ID: {telegram_id}) o‘chirildi!", admin_menu)
...
async def admin_menu_handler(message: types.Message):
    ...
    if not is_super_admin(user_id):
        ...
        await send_response(message, "🚫 Sizda bu bo‘limga kirish huquqi yo‘q!")
        return
    logger.info("Admin menu yuborildi")
    await send_response(message, "Admin paneliga xush kelibsiz! Kerakli bo‘limni tanlang:", admin_menu1)
...
async def back_to_main_menu(message: types.Message):
    ...
    await send_response(message, "Asosiy menyuga qaytdingiz!", admin_menu)
...
async def channels_handler(message: types.Message):
    ...
    await send_response(message, "📢 Kanallar bo‘limi:\n\nBu bo‘limda kanallarni boshqarish mumkin bo‘ladi. Hozircha test rejimida.", admin_menu)
